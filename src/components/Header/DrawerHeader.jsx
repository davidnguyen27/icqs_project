...
import { Drawer, Box, IconButton, Divider, List, ListItem, ListItemText, ListItemIcon } from '@mui/material';
  ...
  return (
    <Drawer anchor="left" open={open} onClose={onClose}>
      <Box role="presentation">
        ...
        >
          <input type="text" placeholder="Enter keyword..." className="search-text" />
        ...
        <List style={{ backgroundColor: '#f5f5f5' }}>
          {navFilter.map((item, index) => (
            <ListItem button key={index} sx={{ justifyContent: 'space-between', marginBottom: '5px' }}>
              <ListItemText primary={item.name} sx={{ my: 0, py: 0, flex: 'none' }} />
              <ListItemIcon sx={{ minWidth: 'auto', mr: 0 }}>{item.icon}</ListItemIcon>
            </ListItem>
          ))}
        </List>
      </Box>
